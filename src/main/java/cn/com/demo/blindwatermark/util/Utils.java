...
public class Utils {
    ...
    public static Mat optimalDft(Mat srcImg) {
        ...
        copyMakeBorder(srcImg, padded, 0, opRows - srcImg.rows(),
                0, opCols - srcImg.cols(), BORDER_REPLICATE);
        return padded;
    }
    ...
    public static void fixSize(Mat watermark, Mat com) {
        if (watermark.rows() != com.rows()) {
            copyMakeBorder(watermark, watermark, 0, com.rows() - watermark.rows(),
                    0, 0, BORDER_CONSTANT, new Scalar(0,0,0,0));
        }
        if (watermark.cols() != com.cols()) {
            copyMakeBorder(watermark, watermark, 0, 0,
                    0, com.cols() - watermark.cols(),BORDER_CONSTANT, new Scalar(0,0,0,0));
        }
    }
